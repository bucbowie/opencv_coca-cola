    ...
    for jpg in os.listdir(scrape_dir):
        try:
            ...
            if os.path.getsize(scrape_dir + "/" + jpg) > size_smallest_img:
                ...
                cv2.imwrite(in_neg_dir + "/" + jpg, resized_image)
            else:
...
in_neg_dir =  '/mnt/usb/in_neg'  #Formatted negative images which will feed 'out_neg.' 
#
#Ensure inputs and outputs exists
check_directories(in_dir)        #Ensure inputs exist before running main logix.
ensure_directories(in_neg_dir)   #Ensure the output directory is defined to hold outputs.
#
#Process images: reading from input; write to output.
#
scrap_raw(scrape_dir)            #Read each image, grayscale and resize - write to outdir.
